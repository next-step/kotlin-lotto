...
- [x] 로또 당첨 번호는 비어 있거나, 중복될 수 없고, `1~45`사이의 정수로 구성된 6개의 숫자여야 하며 `1~45`사이의 정수인 보너스 번호를 가지고 있어야한다.
  - 로또 당첨 번호 리스트 생성시 []을, 보너스 번호에 ``을 넣을 경우 예외(IllegalArgumentException)를 던진다.
  ...
  - 로또 당첨 번호 리스트 생성시 [41, 42, 43, 44, 45]을, 보너스 번호에 ``을 넣을 경우 예외(IllegalArgumentException)를 던진다.
