...
### 기능 목록
- [x] 구입금액은 숫자여야한다.
  - 구입금액에 유효하지 않은 문자열인 "" 입력시 예외(IllegalArgumentException)를 던진다.
  - 구입금액에 유효하지 않은 문자열인 "만원" 입력시 예외(IllegalArgumentException)를 던진다.
- [x] 구입금액은 1000원 이상이여야한다.
  - 구입금액에 "0" 입력시 예외(IllegalArgumentException)를 던진다.
  - 구입금액에 "900" 입력시 예외(IllegalArgumentException)를 던진다.
  - 구입금액에 "1000" 입력시 로또를 1개를 가지고 있어야 한다.
- [x] 구입금액은 1000의 배수여야한다
...
- [x] 지난 주 당첨 번호는 ", "로 구분되며 중복되지 않은 숫자 6개를 입력해야한다.
  ...
  - 지난 주 당첨 번호에 "1, 2, 3, 4, 5, 6, 7" 입력시 예외(IllegalArgumentException)를 던진다.
  - 지난 주 당첨 번호에 "1, 2, 3, 4, 5, 6" 입력시 지난 주 당첨 번호에 [1, 2, 3, 4, 5, 6]이 저장 되어야 한다.
- [x] 지난 주 당첨 번호 입력시 1 ~ 45 사이의 숫자만 입력 할수 있다.
  - 지난 주 당첨 번호에 "0, 1, 2, 3, 4, 5" 입력시 예외(IllegalArgumentException)를 던진다.
  - 지난 주 당첨 번호에 "41, 42, 43, 44, 45, 46" 입력시 예외(IllegalArgumentException)를 던진다.
  - 지난 주 당첨 번호에 "1, 2, 3, 4, 5, 6" 입력시 지난 주 당첨 번호에 [1, 2, 3, 4, 5, 6]이 저장 되어야 한다.
