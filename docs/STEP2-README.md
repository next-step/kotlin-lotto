...
* Domain
  ...
  * [x] 단일 로또(Lotto, 일급 컬렉션) : 6개의 로또 번호로 구성된 `로또 한게임을 표현`한다.
    * [x] LottoNumber 객체의 숫자가 같은 경우 동일한 객체이므로, `중복 없는 난수`를 저장하기 위해 6개의 로또 번호는 `Set<LottoNumber>`의 자료형으로 표현한다. 
    * [x] LottoNumber 객체의 숫자가 같은 경우 동일한 객체이므로, `Set.intersect() 함수를 이용`하여 `당첨 번호와 일치하는 숫자 갯수` 반환 시 `당첨 번호 원소와의 교집합 원소의 갯수`를 반환한다.
  * [ ] 다중 로또(Lottos) : `여러 게임`의 `로또를 표현`한다.
  * [x] 당첨 번호(WinningNumber, 일급컬렉션) : 6개의 로또 번호로 구성된 `당첨 번호를 표현`한다.
    * [x] 당첨 결과(MatchResult) : `일치된 갯수를 이용`하여 1등~6등의 `로또 당첨 등수를 판별`한다.
