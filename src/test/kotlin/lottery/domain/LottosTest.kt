...
class LottosTest {
    ...
    fun `로또 번호 리스트를 가지는 객체를 생성한다`() {
        val result = Lottos(AutoLottoMachine(RandomNumberGenerator()).createLottos(5))
    ...
    fun `로또 번호 리스트를 가지고 있는 객체는 당첨 번호 정보를 가지는 로또 객체를 받아 매칭 결과를 리스트로 반환한다`() {
        ...
        val result = Lottos(ManualLottoMachine.createLottos(lottoNumbers)).matchLottos(winningLotto)
