...
class LottoTicketsTest : BehaviorSpec({
    ...
    Given("티켓 수량이 주어지면") {
        ...
        When("생성 시") {
            val lottoTickets = LottoTickets(lottoTicketAmount = ticketAmount)

            Then("주어진 수량만큼의 로또 티켓이 생성된다") {
                assertEquals(lottoTickets.lottoTicketAmount, 1)
            }
        }
    }

    Given("로또 티켓이 생성되고") {
        val lottoTickets = LottoTickets(lottoTicketAmount = 5)
    ...
    Given("만약 구매금액이 0원일 경우") {
        ...
        When("생성시") {
            val lottoTickets = shouldThrow<IllegalArgumentException> {
                LottoTickets(lottoTicketAmount = buyAmount)
            }
