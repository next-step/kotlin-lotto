...
class LottoNumberValidatorTest : StringSpec({
    "전달된 Set의 길이가 6인지 검사할 수 있다. 6개가 아닐 경우 예외를 반환한다." {
        assertSoftly {
            shouldThrow<IllegalArgumentException> { LottoNumberValidator.validateSize(setOf(1, 2, 3, 4, 5)) }
            shouldThrow<IllegalArgumentException> { LottoNumberValidator.validateSize(setOf(1, 2, 3, 4, 5, 6, 7)) }
        }
    }
