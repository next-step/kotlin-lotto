...
class LottoNumberListTest : FunSpec({
    ...
    test("번호 리스트 생성시 []을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            LottoNumberList.valueOf(listOf())
        }
    }
    ...
    test("로또 번호 리스트 생성시 [1, 2, 3, 4, 5, 6]을 넣을 경우 [1, 2, 3, 4, 5, 6]을 가지고 있는 로또 번호 리스트가 생성 되어야 한다.") {
        val lottoNumberList = LottoNumberList.valueOf(listOf(1, 2, 3, 4, 5, 6))
        lottoNumberList.numberList[0].number shouldBe 1
        lottoNumberList.numberList[1].number shouldBe 2
        lottoNumberList.numberList[2].number shouldBe 3
        lottoNumberList.numberList[3].number shouldBe 4
        lottoNumberList.numberList[4].number shouldBe 5
        lottoNumberList.numberList[5].number shouldBe 6
    }
})
