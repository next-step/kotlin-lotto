...
class ResultLottoTest {
    ...
    fun `1부터 45가 아닌 숫자를 주면 IllegalArgumentException throw`() {
        ...
        shouldThrow<IllegalArgumentException> {
            ResultLotto.from(Lotto.from(numbers), LottoNumber.build(bonusNumber))
        }
    }
    ...
    fun `2등을 위한 보너스 숫자를 받는다`() {
        val numbers = setOf(1, 2, 3, 4, 5, 6)
        val bonusNumber = 7
        ResultLotto.from(Lotto.from(numbers), LottoNumber.build(bonusNumber)).bonusNumber.value shouldBe bonusNumber
    }
}
