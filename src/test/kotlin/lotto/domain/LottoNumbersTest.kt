...
internal class LottoNumbersTest : BehaviorSpec({
        ...
        `when`("6개가 아니라면") {
            ...
            then("IllegalArgumentException 이 발생한다.") {
                shouldThrow<IllegalArgumentException> {
                    LottoNumbers(lottoNumbers.toSet())
                }
            }
        }
    ...
    given("당첨 번호와 구입한 로또의 일치하는 수의 개수를 구할 때") {
        ...
        `when`("3개가 일치한다면") {
            val purchasedLottoNumbersNumbers = LottoNumbers(
                setOf(
                    LottoNumber(1),
                    LottoNumber(2),
                    LottoNumber(3),
            ...
            then("일치하는 수 3을 반환한다.") {
                val result = winningLottoNumbersNumbers.getNumberOfMatch(purchasedLottoNumbersNumbers)
                result shouldBe 3
            }
        }
    }
