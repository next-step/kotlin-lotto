    ...
    given("로또리스트와 당첨번호가 주어졌다") {
        val lottos = Lottos(
            listOf(
                ...
                Lotto(
                    (LottoNumber.LOTTO_MIN_NUMBER + 1 until LottoNumber.LOTTO_MIN_NUMBER + Lotto.LOTTO_NUMBER_SIZE).toSortedSet() + sortedSetOf(
                        LottoNumber.LOTTO_MIN_NUMBER + Lotto.LOTTO_NUMBER_SIZE + 2
                    )
                ), // 3등
                Lotto((LottoNumber.LOTTO_MIN_NUMBER + 2 until LottoNumber.LOTTO_MIN_NUMBER + Lotto.LOTTO_NUMBER_SIZE + 2).toSortedSet()), // 4등
                Lotto((LottoNumber.LOTTO_MIN_NUMBER + 3 until LottoNumber.LOTTO_MIN_NUMBER + Lotto.LOTTO_NUMBER_SIZE + 3).toSortedSet()), // 5등
        ...
        val winNumbers =
            WinNumbers(
                ...
                LottoNumber.LOTTO_MIN_NUMBER + Lotto.LOTTO_NUMBER_SIZE
            )
        `when`("당첨결과를 확인했을때") {
            then("정확한 결과가 반환된다") {
