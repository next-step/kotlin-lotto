...
class LottoTicketsTest : DescribeSpec({
    describe("init 메서드 테스트") {
        context("로또 티켓 개수가 1개 이상 일 떄,") {
            val givenLottoTickets = listOf(LottoTicket.of(setOf(1, 2, 3, 4, 5, 6)))
    ...
    describe("getMatchCount 메서드") {
        val givenLottoTicket1 = LottoTicket.of(setOf(7, 9, 11, 34, 35, 43))
        val givenLottoTicket2 = LottoTicket.of(setOf(5, 6, 9, 13, 34, 35))
        val givenLottoTickets = LottoTickets(listOf(givenLottoTicket1, givenLottoTicket2))

        context("로또 랭크가 THIRD 일 때") {
            val givenLottoWinningNumber = LottoWinningNumber(LottoTicket.of(setOf(5, 6, 9, 13, 34, 1)), LottoNumber.of(2))
        ...
        context("로또랭크가 SECOND 일 때") {
            it("보너스 번호가 동일하면 매치 카운트를 한다.") {
                val givenLottoWinningNumber = LottoWinningNumber(LottoTicket.of(setOf(5, 6, 9, 13, 34, 1)), LottoNumber.of(35))
            ...
            it("보너스 번호가 동일하지 않다면 카운트하지 않는다.") {
                val givenLottoWinningNumber = LottoWinningNumber(LottoTicket.of(setOf(5, 6, 9, 13, 34, 1)), LottoNumber.of(36))
                val matchCount = givenLottoTickets.getMatchCount(LottoRank.SECOND, givenLottoWinningNumber)
