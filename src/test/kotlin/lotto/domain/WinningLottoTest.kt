...
class WinningLottoTest : StringSpec({
    ...
    "일치 하는 로또의 수를 알 수 있다." {
        ...
        winningLotto.correctCount(lotto) shouldBe 6
    }
    ...
    "각 등수 정상 결과 확인" {
        forAll(
            ...
            winningLotto.correctCount(Lotto.from(lottoNumber)) shouldBe correctCount
        }
    }
    ...
    "보너스 볼은 6자리 로또 번호와 달라야 한다." {
        val exception = shouldThrow<IllegalArgumentException> {
            WinningLotto(
                Lotto.from("1,2,3,4,5,6"), LottoNumber(6)
            )
        }
    ...
    "보너스 볼 포함 여부 확인" {
        winningLotto.matchedBonusBall(Lotto.from("1,2,3,4,5,7")) shouldBe true
    }
})
