...
class LottoWinTicketTest : FunSpec({
    ...
    context("LottoWinTicket 생성") {
        test("fun from()") {
            shouldNotThrowAny {
                LottoWinTicket.from("1,2,3,4,5,6")
            }
        }
    }

    context("LottoWinTicket 메소드") {
        val lottoWinTicket = LottoWinTicket.from("1,2,3,4,5,6")

        test("matches") {
            with(lottoWinTicket) {
                matches(LottoNumbers.from("1,2,3,4,5,6")) shouldBe LottoReward.FIRST
                matches(LottoNumbers.from("7,2,3,4,5,6")) shouldBe LottoReward.SECOND
                matches(LottoNumbers.from("7,8,3,4,5,6")) shouldBe LottoReward.THIRD
                matches(LottoNumbers.from("7,8,9,4,5,6")) shouldBe LottoReward.FIRTH
