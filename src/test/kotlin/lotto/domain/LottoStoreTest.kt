...
class LottoStoreTest : FreeSpec({
    "입력된 금액에 따라 로또를 여러장 구매한다" {
        val lottoAmount = LottoAmount(14000)

        val lottos = LottoStore.buy(ManualLottos(listOf()), lottoAmount)
    ...
    "수동과 자동을 섞어서 입력된 금액의 개수만큼 구매한다" {
        ...
        val userLottos = LottoStore.buy(manualLottos, lottoAmount)
    ...
    "수동을 입력 금액만큼 구매할 수 있다" {
        val lottoAmount = LottoAmount(3000)
        val lottos =
            listOf(
                Lotto(LottoNumbers(1, 2, 3, 4, 5, 6)),
                Lotto(LottoNumbers(1, 2, 3, 4, 5, 6)),
