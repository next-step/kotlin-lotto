...
class TicketTest {
    ...
    fun `countMatches() 동일한 숫자 갯수를 찾는다`() {
        assertThat(
            Ticket(listOf(1, 2, 3, 4, 5, 6)).countMatches(
                Ticket(listOf(6, 5, 4, 3, 2, 1))
            )
        ).isEqualTo(6)

        assertThat(
            Ticket(listOf(1, 2, 3, 4, 5, 6)).countMatches(
                Ticket(listOf(16, 5, 4, 3, 2, 1))
            )
        ).isEqualTo(5)

        assertThat(
            Ticket(listOf(1, 2, 3, 4, 5, 6)).countMatches(
                Ticket(listOf(16, 15, 4, 3, 2, 1))
            )
        ).isEqualTo(4)

        assertThat(
            Ticket(listOf(1, 2, 3, 4, 5, 6)).countMatches(
                Ticket(listOf(16, 15, 14, 13, 12, 11))
            )
        ).isEqualTo(0)
    }
}
