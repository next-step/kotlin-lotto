...
class TicketMachineTest : StringSpec({
    "수동번호 입력이 없는 경우 구입금액을 입력하면 자동 티켓 카운트를 반환한다" {
        exchange(Money(2000), emptyList()).autoTickets.size shouldBe 2
    }

    "금액을 1,000원단위로 하지 않을 시 예외 발생한다" {
        listOf(1100, 1200, 3500, 12310, 1500).forAll { amount ->
            shouldThrow<IllegalArgumentException> { exchange(Money(amount), emptyList()) }
        }
    }
