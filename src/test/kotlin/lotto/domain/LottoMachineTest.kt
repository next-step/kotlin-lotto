...
class LottoMachineTest {
    ...
    fun `구매 로또와 당첨 로또를 받았다면, 당첨 확인을 요청했을 때, 당첨 등수를 반환한다`() {
        ...
        val winningLotto = WinningLotto(Lotto(LottoNumber.createLottoNumbers(numberCombination)), bonusLottoNumber)

        val purchaseNumberCombination = NumberCombination(listOf(1, 2, 3, 4, 5, 8))
        val purchaseLotto = Lotto(LottoNumber.createLottoNumbers(purchaseNumberCombination))
    ...
    fun ` 2등 당첨 시나리오`() {
        ...
        val purchaseNumberCombination = NumberCombination(listOf(1, 2, 3, 4, 5, 7))
        val purchaseLotto = Lotto(LottoNumber.createLottoNumbers(purchaseNumberCombination))
    ...
    fun `당첨 로또와 보너스 번호를 받고, 당첨 로또 번호 생성을 요청할때, 당첨 로또번호가 반환된다`() {
        ...
        val numberCombination = NumberCombination(listOf(1, 2, 3, 4, 5, 6))
        val lottoNumber = LottoNumber.createLottoNumbers(numberCombination)
        val bonusNumber = 8

        // when : 당첨 번호
        val actual = LottoMachine.createWinningLotto(numberCombination, bonusNumber)
