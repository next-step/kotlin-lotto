...
internal class LottoMarketTest {
    ...
    fun `로또를 수동으로 구매할 수 있다`() {
        ...
        val lottoPaper = LottoMarket.buyManual(user, listOf(Lotto(1, 2, 3, 4, 5, 6)))
    ...
    fun `돈이 모자라면 로또를 수동으로 구매할 수 없다`() {
        ...
        assertThrows<IllegalStateException> { LottoMarket.buyManual(user, listOf(Lotto(1, 2, 3, 4, 5, 6))) }
    }
    ...
    fun `로또를 자동으로 구매할 수 있다`() {
        ...
        val lottoPaper = LottoMarket.buyMaxAutomation(user)
