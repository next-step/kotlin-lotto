...
import lotto.domain.money.BuyMoney.Companion.DEFAULT_LOTTO_GAME_PRICE
...
internal class BuyMoneyTest {
    ...
    @DisplayName("티켓 기본가인 ${DEFAULT_LOTTO_GAME_PRICE}의 배수로 구성된 값이 주어지면 `Money` 객체 생성")
    @ParameterizedTest
    @ValueSource(ints = [1_000, 2_000, 10_000, 15_000, 33_000, 78_000, 100_000, 1_500_000])
    fun `create Money if given value that is composed of a multiple of default lotto game price`(givenMoney: Int) {
        // Arrange
        // Act
    ...
    @DisplayName("주어지는 값이 ${DEFAULT_LOTTO_GAME_PRICE}의 배수가 아닌 경우 `Money` 객체 생성 실패")
    @ParameterizedTest
    @ValueSource(ints = [100, 390, 999, 1_205, 432_128, 100_000_001])
    fun `create Money if given value that is not composed of a multiple of default lotto game price`(givenMoney: Int) {
        // Arrange
        // Act
        // Assert
        assertThatThrownBy() {
