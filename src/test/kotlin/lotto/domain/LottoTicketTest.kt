...
class LottoTicketTest {
    @Test
    fun `로또 번호는 6개다`() {
        assertThat(LottoTicket(LottoRandomNumbers().createNumbers()).numbers.size).isEqualTo(6)
    }

    @Test
    fun `로또 번호는 정렬되어 있다`() {
        val lottoNumbers = LottoFixedNumbers().createNumbers(listOf(6, 5, 4, 3, 2, 1))
        assertThat(LottoTicket(lottoNumbers).numbers).isEqualTo(setOf(1, 2, 3, 4, 5, 6))
    }

    @Test
    fun `로또 번호 입력으로 6개 미만의 숫자열이 들어가면 예외가 발생한다`() {
        Assertions.assertThatThrownBy {
            LottoTicket(setOf(LottoNumber(1)))
        }
    ...
    fun `로또 번호는 각각 1 이상 45 이하이다`() {
        ...
        assertThat(LottoTicket(lottoNumbers).numbers).allMatch { number -> number in 1..45 }
    }
