...
import lotto.strategy.FixedNumberListGenerator
import lotto.stretagy.NumberListGenerator

class LottoTest : DescribeSpec({
    describe("로또를 생성한다") {
        lateinit var numberListGenerator: NumberListGenerator

        beforeTest { numberListGenerator = FixedNumberListGenerator() }

        it("로또를 생성할 개수를 받아 로또를 생성한다") {
            val sut = Lotto.generate(numberListGenerator, 10)
            sut.size shouldBe 10
        }

        it("다른 숫자로 이루어진 6자리 로또를 생성한다") {
            val actual = Lotto.generate(numberListGenerator, 1)
            val lotto: Lotto = actual[0]
            lotto.lottoNumbers.size shouldBe 6
        }

        it("로또의 각 숫자는 오름차순으로 정렬 되어 있어야 한다.") {
            val lotto = Lotto.generate(numberListGenerator, 1)[0]
