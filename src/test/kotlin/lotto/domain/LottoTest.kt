...
class LottoTest {
    ...
    fun `정수 배열의 길이가 5 이하인 경우 에러를 반환한다`(numbers: Set<Int>) {
    ...
    fun `정수 배열의 길이가 6이며 중복된 숫자가 없는 배열을 넣으면 로또 객체가 생성된다`() {
        assertThat(Lotto(setOf(1, 2, 3, 4, 5, 6))).isExactlyInstanceOf(Lotto::class.java)
    }
    ...
    fun `정수 배열의 길이가 7 이상인 경우 에러를 반환한다`(numbers: Set<Int>) {
        assertThrows<IllegalArgumentException> {
            Lotto(numbers)
        }
    }
    ...
    fun `toString()은 로또의 numbers 배열을 출력한다`() {
        assertThat(Lotto(setOf(1, 2, 3, 4, 5, 6)).toString()).isEqualTo(setOf(1, 2, 3, 4, 5, 6).toString())
    }
    ...
    companion object {
        ...
        fun generateUnder6Source(): List<Arguments> {
            return listOf(
                Arguments.of(setOf<Int>()),
                Arguments.of(setOf(1)),
                Arguments.of(setOf(1, 2)),
                Arguments.of(setOf(1, 2, 3)),
                Arguments.of(setOf(1, 2, 3, 4)),
        ...
        fun generateOver7Source(): List<Arguments> {
            return listOf(
                ...
                Arguments.of(setOf(1, 2, 3, 4, 5, 6, 7, 8)),
            )
        }
    }
}
