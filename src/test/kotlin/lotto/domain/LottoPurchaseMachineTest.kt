    ...
    given("금액을 지불한다.") {
        `when`("수동 로또 번호를 전달한다") {
            then("남은 금액만큼의 자동로또를 생성한다.") {
                forAll(
                    row(6000, ObjectGenerator.randomLotto(2), 6),
                    row(14500, ObjectGenerator.randomLotto(2), 14),
                    row(0, ObjectGenerator.randomLotto(0), 0),
        ...
        `when`("지불한 금액보다 더 높은 금액의 수동로또 번호를 전달한다.") {
            then("예외가 발생한다.") {
                ...
                val manualLottos = ObjectGenerator.randomLotto(3)
