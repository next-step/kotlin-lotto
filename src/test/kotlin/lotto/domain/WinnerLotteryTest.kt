...
class WinnerLotteryTest : StringSpec({
    "bonus 값은 1~45 사이의 정수가 가능" {
        (7..45).forEach {
            shouldNotThrowAny {
                WinnerLottery(LottoNumber(setOf(1, 2, 3, 4, 5, 6)), bonus = it)
            }
        }
    }
    ...
    "당첨 등수 구하기" {
        // given
        val winnerLottery = WinnerLottery(LottoNumber(setOf(1, 2, 3, 4, 5, 6)), bonus = 7)
        table(
            headers("로또 번호", "expect", "description"),
            row(Lottery(LottoNumber(setOf(1, 2, 3, 4, 5, 6))), WinnerPrize.FIRST_PRIZE, "1등"),
            row(Lottery(LottoNumber(setOf(1, 2, 3, 4, 5, 7))), WinnerPrize.SECOND_PRIZE, "2등"),
            row(Lottery(LottoNumber(setOf(1, 2, 3, 4, 5, 8))), WinnerPrize.THIRD_PRIZE, "3등"),
            row(Lottery(LottoNumber(setOf(1, 2, 3, 4, 7, 8))), WinnerPrize.FOURTH_PRIZE, "4등")
