...
class WinningTicketTest {
    @Test
    fun `당첨티켓의 번호는 6 자리다`() {
        val winningTicket = WinningTicket(LottoTicket(LottoRandomNumbers().createNumbers()))
    ...
    fun `당첨티켓은 보너스 번호를 가질 수 있다`() {
        val winningTicket = WinningTicket(LottoTicket(LottoRandomNumbers().createNumbers()))
        winningTicket.chooseBonusBall("10")
        assertThat(winningTicket.bonusNumber).isEqualTo(10)
    }

    @Test
    fun `당첨티켓의 보너스 번호가 로또 번호와 중복될 경우 에러가 발생한다`() {
        val lottoNumbers = LottoFixedNumbers().createNumbers(listOf(1, 2, 3, 4, 5, 6))
        val winningTicket = WinningTicket(LottoTicket(lottoNumbers))

        Assertions.assertThatThrownBy {
            winningTicket.chooseBonusBall("6")
        }
