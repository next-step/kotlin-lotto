...
class LottoMachineTest {
    ...
    fun `번호 리스트를 받았다면, 로또를 생성 요청을 했을 때, 해당 번호로 구성된 로또를 생성한다`() {
        // given : 번호를 입력받는다면
        val numberList = sortedSetOf(1, 2, 3, 4, 5, 6)

        // when : 로또를 생성한다.
        val actual = LottoMachine.createLotto(numberList.toList())
    ...
    fun `구매 로또와 추첨된 로또를 받았다면, 당첨 확인을 요청했을 때, 당첨 등수를 반환한다`() {
        ...
        val winningLotto = Lotto(sortedSetOf(1, 2, 3, 4, 5, 6))
        val purchaseLotto1 = Lotto(sortedSetOf(1, 2, 3, 4, 5, 8))
