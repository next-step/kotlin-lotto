...
class WinningLottoNumberListTest : FunSpec({
    ...
    test("지난 주 당첨 로또 번호 리스트 생성시 [1, 2, 3, 4, 5]을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            WinningLottoNumberList.valueOf("1, 2, 3, 4, 5")
        }
    }

    test("지난 주 당첨 로또 번호 리스트 생성시 [1, 2, 3, 4, 5, 5]을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            WinningLottoNumberList.valueOf("1, 2, 3, 4, 5, 5")
        }
    }

    test("지난 주 당첨 로또 번호 리스트 생성시 [1, 2, 3, 4, 5, 6, 7]을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            WinningLottoNumberList.valueOf("1, 2, 3, 4, 5, 6, 7")
        }
    }

    test("지난 주 당첨 로또 번호 리스트 생성시 [0, 1, 2, 3, 4, 5]을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            WinningLottoNumberList.valueOf("0, 1, 2, 3, 4, 5")
        }
    }

    test("지난 주 당첨 로또 번호 리스트 생성시 [41, 42, 43, 44, 45, 46]을 넣을 경우 예외(IllegalArgumentException)를 던진다.") {
        shouldThrow<IllegalArgumentException> {
            WinningLottoNumberList.valueOf("41, 42, 43, 44, 45, 46")
        }
    }

    test("지난 주 당첨 로또 번호 리스트 생성시 [1, 2, 3, 4, 5, 6]을 넣을 경우 지난 주 당첨 로또 번호 리스트에 [1, 2, 3, 4, 5, 6]이 저장 되어야 한다.") {
