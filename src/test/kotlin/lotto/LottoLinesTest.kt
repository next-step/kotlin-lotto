...
class LottoLinesTest {
    ...
    fun `로또 티켓(여러줄) 생성 테스트-수동 생성`(inputMoney: Int, manualCount: Int, gameCount: Int) {
        ...
        Assertions.assertThat(lottoTicket.getLines().filter { it.manual }.size).isEqualTo(manualCount)
        Assertions.assertThat(lottoTicket.getLines().filter { !it.manual }.size).isEqualTo(gameCount - manualCount)
    }
}
