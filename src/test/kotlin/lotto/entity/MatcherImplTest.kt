...
internal class MatcherImplTest {
    ...
    fun `4개의 숫자가 맞고 보너스 볼이 맞지 않은 경우 4등을 한다`() {
        ...
        // when
        val matchingResult = MatcherImpl(winningNumbers, bonus).matchOneTicket(ticket)
    ...
    fun `5개의 숫자가 맞고 보너스 볼이 맞지 않는 경우 3등을 한다`() {
        // given
        val winningNumbers = listOf<Int>(1, 2, 3, 4, 5, 6)
        val lottoNumbers = listOf<Int>(1, 2, 3, 4, 5, 8)
        val ticket = LottoTicket(lottoNumbers)
        val bonus = 12
    ...
    fun `2개 5개가 맞는 티켓을 한 번에 매칭하면 3등, 맞지 않는 결과가 나온다`() {
        ...
        // when
        val matchingResultList = MatcherImpl(winningNumbers, bonus).matchAllTickets(tickets)
    ...
    fun `matchTicketsToRanks 함수 - 2개 5개 4개 일치하는 티켓이 있다면 mapOf(3 to 0, 4 to 1, 5 to 1, 6 to 0)를 리턴한다`() {
        ...
        // when
        val winningRanks = MatcherImpl(winningNumbers, bonus).countTicketRanks(tickets).countOfRanks
