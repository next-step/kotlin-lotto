...
class LottoTest {
    ...
    fun `6개로 구성되지 않는 번호 구성을 받았다면, 로또를 생성했을 때, 예외를 던진다`() {
        ...
        val selectNumberCombination = NumberCombination(listOf(1, 2, 3, 4, 5, 6, 7))

        // when : 로또번호를 구성한다.
        val actual = runCatching { Lotto(LottoNumber.createLottoNumbers(selectNumberCombination)) }.exceptionOrNull()
    ...
    fun `중복값을 포함한 6개의 번호 구성을 받았다면, 로또를 생성했을 때, 예외를 던진다`() {
        ...
        val selectNumberCombination = NumberCombination(listOf(1, 2, 3, 4, 6, 6))
