...
class LottoShopTest {
    ...
    fun `구매한 로또 개수만큼 발권되는지 확인한다`(ticketingCount: Int) {
        assertThat(LottoShop.createLottoTicket(ticketingCount, listOf()).size).isEqualTo(ticketingCount)
    }

    @ParameterizedTest
    @ValueSource(strings = ["1", "2", "15"])
    fun `발권된 로또가 로또 범위(1~45)에 존재하는지 확인한다`(ticketingCount: Int) {
        LottoShop.createLottoTicket(ticketingCount, listOf()).forEach { lotto ->
            lotto
                .getLottoNumber()
