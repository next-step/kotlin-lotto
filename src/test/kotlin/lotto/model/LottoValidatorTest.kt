...
class LottoValidatorTest : BehaviorSpec({
    Given("티켓 3개 리스트, 당첨 번호") {
        val lottoTickets = listOf<LottoTicket>(
            LottoTicket(numbers = listOf(1, 2, 3, 4, 5, 6).map { LottoNumber(it) }),
            LottoTicket(numbers = listOf(3, 4, 5, 6, 7, 8).map { LottoNumber(it) }),
            LottoTicket(numbers = listOf(5, 6, 7, 8, 9, 10).map { LottoNumber(it) })
        )
