...
class LottoOrderTest : StringSpec({
    ...
    "자동으로 구매한 수량만큼 로또게임이 만들어진다" {
        val actual = LottoOrder(2, listOf())
    ...
    "발권이 불가능한 문자열이 입력되면 IllegalArgumentException throw 해야한다" {
        shouldThrow<IllegalArgumentException> {
            println("숫자가 5개라서 실패")
            LottoOrder(3, listOf(Game(LottoNumbers(1, 2, 3, 4, 5))))
        }
        shouldThrow<IllegalArgumentException> {
            println("숫자가 7개라서 실패")
            LottoOrder(3, listOf(Game(LottoNumbers(1, 2, 3, 4, 5, 6, 7))))
        }
    }

    "구매할 로또 Game 하나의 가격은 1000 이다" {
        LottoOrder.priceOfGame() shouldBe 1000
    }
})
