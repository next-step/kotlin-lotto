...
class LottoGradeTest : StringSpec({
    "로또 등급은 5를 제외한 당천 번호 객수로 찾으면 해당하는 등급을 반환한다" {
        listOf(
        ...
        ).forAll() { (correctNumber, grade) ->
            listOf(true, false).forAll { matchPlus ->
    ...
    "로또 등급은 당천 번호 객수로 찾으면 해당하는 등급을 반환한다" {
        listOf(
            5 to LottoGrade.SECOND_GRADE, 5 to LottoGrade.SECOND_PLUS_GRADE
        ).forAll() {
