...
class LottoAgentTest {
    ...
    class LottoAgent(val exchange: Exchange<LottoNumber>) {
        constructor(lottoExchange: LottoDrawMachine) : this(LottoExchange(lottoExchange))

        class LottoExchange(lottoExchange: LottoDrawMachine) : Exchange<LottoNumber> {
            override fun pay(money: Money) {
            }

            override fun product(): List<LottoNumber> = listOf()
        }
    }
