...
internal class NumbersTest {
    ...
    fun `번호는 오름차순 정렬되어 있다`() {
        val numbers = Numbers(sortedSetOf(1, 2, 4, 3, 5, 6))

        assertThat(numbers.values).isEqualTo(setOf(1, 2, 3, 4, 5, 6))
    }
    ...
    fun `로또 번호가 다른 번호와 얼마나 일치하는지 알 수 있다`() {
        val numbers = Numbers(sortedSetOf(1, 2, 3, 4, 5, 7))
        val winningNumbers = Numbers(sortedSetOf(1, 2, 3, 4, 5, 6))
