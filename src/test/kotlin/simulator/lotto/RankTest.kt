...
internal class RankTest {
    ...
    @Test
    fun `숫자 다섯개가 일치하고 보너스 숫자는 일치하지 않다면 THIRD(3등)이며 상금은 15_000_000이다`() {
        assertThat(Rank.valueOf(5, false)).isEqualTo(Rank.THIRD)
        assertThat(Rank.valueOf(5, true)).isNotEqualTo(Rank.THIRD)
        assertThat(Rank.THIRD.winningMoney).isEqualTo(15_000_000)
    }
    ...
    fun `숫자 세개가 일치하면 FIFTH(5등)이며 상금은 5_000이다`() {
        ...
        assertThat(Rank.FOURTH.winningMoney).isEqualTo(5_000)
    }
    ...
    fun `맞는 로또 갯수가 2개 이하라면 MISS이며 상금은 없다`() {
        ...
        assertThat(Rank.MISS.winningMoney).isEqualTo(5_000)
    }
