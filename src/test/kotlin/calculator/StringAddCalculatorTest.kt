...
class StringAddCalculatorTest : StringSpec({
    val expressionFactory = DefaultExpressionFactory()

    "null 인 값을 입력하면 0을 반환한다" {
        val input = null
        val expression = expressionFactory.createExpression(input)
        StringAddCalculator(expression).add() shouldBe 0
    }
    ...
    "숫자 하나를 문자열로 입력할 경우 해당 숫자를 반환한다." {
        val input = "1"
        val expression = expressionFactory.createExpression(input)
        StringAddCalculator(expression).add() shouldBe 1
    }
    ...
    "분리된 숫자의 합을 반환한다." {
        val input = "1,2:3"
        val expression = expressionFactory.createExpression(input)
        StringAddCalculator(expression).add() shouldBe 6
    }

    "기본 구분자 외에 커스텀 구분자를 지정할 수 있다." {
        val input = "//;\n1;2;4"
        val expression = expressionFactory.createExpression(input)
        StringAddCalculator(expression).add() shouldBe 7
    }
})
