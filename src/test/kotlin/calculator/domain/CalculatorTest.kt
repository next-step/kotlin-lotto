    ...
    given("덧셈 계산기에 문자열을 입력할 때") {
        `when`("커스텀 구분자를 포함하고") {
            ...
            and("설정한 구분자로 구분된 숫자를 가진 문자열을 입력하면") {
                val numbers = listOf(1, 11, 2)
                val numbersString = numbers.joinToString(CUSTOM_SEPARATOR)

                then("기대한 값을 반환한다.") {
                    val result = calculator.run(customSeparatorString + numbersString)
        ...
        `when`("커스텀 구분자를 포함하지 않은 구분된 숫자를 입력하면") {
            ...
            then("기대한 값을 반환한다.") {
                val result = calculator.run(string)
                result shouldBe 79
            }
        }
        ...
        `when`("숫자 하나만 입력하면") {
            val number = "111"
            then("입력한 숫자를 반환한다.") {
                val result = calculator.run(number)
                result shouldBe 111
            }
        }
    }
