...
class LottoTest : FunSpec({
    ...
    test("자동으로 생성된 번호는 6개이다") {
        ...
        lotto.numberList.numbers.count() shouldBe expected
    }

    test("생성된 번호는 겹치지 않는다") {
        val lottoSet = lotto.numberList.numbers.toSet()

        lotto.numberList.numbers.count() shouldBe lottoSet.count()
    }
    ...
    test("로또 생성시에 번호를 전달하면, 전달된 대로 생성한다") {
        ...
        winningLotto.numberList.numbers shouldBe expected
    }
