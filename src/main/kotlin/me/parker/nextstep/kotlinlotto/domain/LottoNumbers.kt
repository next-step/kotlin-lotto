...
class LottoNumbers(val values: List<LottoNumber>) {
    init {
        require(values.size == LOTTO_NUMBERS_SIZE) { "로또 번호는 $LOTTO_NUMBERS_SIZE 개를 가지고 있어야합니다." }
        require(values.toSet().size == LOTTO_NUMBERS_SIZE) { "로또 번호는 중복되지 않아야 합니다." }
    }

    constructor(generationRule: LottoNumbersGenerationRule) : this(generationRule.generate())
