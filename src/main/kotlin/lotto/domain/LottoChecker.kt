...
object LottoChecker {
    ...
    fun checkResult(lottos: List<Lotto>, winNumber: List<Int>): Map<Lotto.WinningPrize, Int> {
        ...
        val resultMap = mutableMapOf<Lotto.WinningPrize, Int>()
    ...
    private fun MutableMap<Lotto.WinningPrize, Int>.addResult(lotto: Lotto, winNumber: List<Int>) {
        val count = winNumber.count { number -> number in lotto.numbers }
        if (count >= Lotto.LOTTO_WINNING_MIN_COUNT) {
            val winningAmount = Lotto.WinningPrize.of(count)
