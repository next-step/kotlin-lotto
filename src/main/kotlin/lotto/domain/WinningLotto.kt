...
class WinningLotto(private val winningNumbers: Lotto, private val bonusNumber: LottoNumber) {
    ...
    fun getMatchResult(lottos: List<Lotto>): Map<Rank, Int> {
        val countOfMatchRank = getCountOfMatchRank(lottos)
        return Rank.values()
            .associateWith { countOfMatchRank[it] ?: 0 }
    }

    private fun getCountOfMatchRank(lottos: List<Lotto>): Map<Rank, Int> {
        return lottos.groupingBy { Rank.valueOf(getCountOfMatch(it), matchBonus(it)) }.eachCount()
    }
