...
object LottoMatcher {
    ...
    fun matchingWinner(matchNumberCount: Int, bonus: Boolean= false): Winner {
        check((Lotto.START_LOTTO_INDEX..Lotto.LAST_LOTTO_INDEX).contains(matchNumberCount)) { "당첨 개수는 최대 6개 최소 0개 입니다" }
        return Winner.safeValueOf(matchingCount = matchNumberCount, bonus = bonus)
    }
    ...
    fun matchBonus(matchCount: Int, lottoNumbers: List<Int>, bonusNumber: Int): Boolean {
        return matchCount == FIVE_MATCHING_COUNT && lottoNumbers.contains(bonusNumber)
    }
