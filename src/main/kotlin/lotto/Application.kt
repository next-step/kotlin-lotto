...
fun main() {
    ...
    val manualLottos = InputView.inputManualNumbers(manualCount, LottoProgram.isNotDuplicated()).map { it }
    val lottos = Lottos.newInstance(amountOfLotto, manualLottos)
    OutputView.showUserLottos(lottos.lottos, manualCount)
    val numbers = InputView.inputWinningNumbers(LottoProgram.isNotDuplicated())
    val bonusBall = InputView.inputBonusBall(LottoProgram.validateBonusBallRange())
