...
object ResultView {
    ...
    fun printReturn(lottoResult: LottoResult) {
        println("당첨 통계")
        println("---------")
        println("3개 일치 (${LottoReturn.FOURTH.returnPrice}원)- ${lottoResult.count(LottoReturn.FOURTH)}개")
        println("4개 일치 (${LottoReturn.THIRD.returnPrice}원)- ${lottoResult.count(LottoReturn.THIRD)}개")
        println("5개 일치 (${LottoReturn.SECOND.returnPrice}원)- ${lottoResult.count(LottoReturn.SECOND)}개")
